...
describe('AyahOfTheDay Component', () => {
  beforeEach(() => {
    ...
    (corpusStore.getLocalSurahData as any).mockImplementation((_surahId: number) => ({
      ayahs: [
        { numberInSurah: 28, textAr: 'DefaultArabic', textSq: 'DefaultAlbanian' },
        { numberInSurah: 286, textAr: 'DefaultArabic', textSq: 'DefaultAlbanian' },
        { numberInSurah: 5, textAr: 'DefaultArabic', textSq: 'DefaultAlbanian' },
      ]
    }));
  });
  ...
  it('selects verse key deterministically based on date', async () => {
    // 2024-01-01 -> hash % 7 === 1 ("2:286")
    (dateUtils.getLocalDateString as any).mockReturnValue('2024-01-01');
    (corpusStore.getLocalSurahData as any).mockImplementation(() => ({
      ayahs: [{ numberInSurah: 286, textAr: 'Text1', textSq: 'Trans1' }]
    }));
    ...
    // 2024-01-02 -> hash % 7 === 2 ("94:5")
    (dateUtils.getLocalDateString as any).mockReturnValue('2024-01-02');
    (corpusStore.getLocalSurahData as any).mockImplementation(() => ({
      ayahs: [{ numberInSurah: 5, textAr: 'Text2', textSq: 'Trans2' }]
    }));
  ...
  it('fetches verse from local corpus if available', async () => {
    ...
    (dateUtils.getLocalDateString as any).mockReturnValue('2024-01-01');
    (corpusStore.getLocalSurahData as any).mockImplementation(() => ({
      ayahs: [{ numberInSurah: 286, textAr: 'LocalArabic', textSq: 'LocalAlbanian' }]
    }));
  ...
  it('calls onOpenAyah when button is clicked', async () => {
    ...
    expect(mockOnOpen).toHaveBeenCalledWith(2, 286);
  });
});
